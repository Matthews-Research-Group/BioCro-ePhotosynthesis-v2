...
plot_print <- function(x) {
    if (SAVE_TO_PDF) {
        pdf(file = paste0('Rplots', sprintf('%03d', plot_num), '.pdf'))
        print(x)
        dev.off()
        plot_num <<- plot_num + 1
    } else {
...
file_paths <- c(
    "2022-07-21 mcgrath1.xlsx",
    "2022-07-21 mcgrath2.xlsx",
    "2022-08-03 ed aci ripe2.xlsx",
    "2022-08-03 ed aci ripe3.xlsx",
    "2022-08-03 ed aci ripe14.xlsx",
    "2022-08-03 ed aci ripe15.xlsx",
    "2022-08-05 ripe2 aci.xlsx",
    "2022-08-05 ripe3 aci.xlsx",
    "2022-08-05 ripe14 aci.xlsx",
    "2022-08-05 ripe15 aci.xlsx"
)
...
# Export curves
write.csv(licor_data[licor_data[, 'species'] == 'ld10', , TRUE], 'aci_soybean_ld10_2022.csv', row.names = FALSE)
write.csv(licor_data[licor_data[, 'species'] == 'ld11', , TRUE], 'aci_soybean_ld11_2022.csv', row.names = FALSE)
...
point_colors <- multi_curve_point_colors()
...
line_colors <- multi_curve_line_colors()
...
invisible(lapply(box_bar_plot_param, function(x) {
  plot_print(do.call(bwplot_wrapper, x))
  plot_print(do.call(barchart_with_errorbars, x))
}))
...
write.csv(as.data.frame(soybean_fvcb_parameters), file = 'soybean_ld10ld11_fvcb_parameters_2022.csv', row.names = FALSE)
