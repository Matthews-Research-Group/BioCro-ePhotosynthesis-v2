...
make_plot1<-function(){
  ...
  plot(fitting_results$step, fitting_results$sf1,type='b',
       lwd = linewidth,
       xlab='',ylab='')
  title(xlab = "iterations", line = axislabelspace)           # Add x-axis text
  title(ylab = "sf (PhiPS2 base)", line = axislabelspace)                 # Add y-axis text
  
  plot(fitting_results$step, fitting_results$sf2,type='b',
       lwd = linewidth,
       xlab='',ylab='')
  title(xlab = "iterations", line = axislabelspace)           # Add x-axis text
  title(ylab = "sf (Theta base)", line = axislabelspace)                 # Add y-axis text
  
  plot(fitting_results$step, fitting_results$rmse,type='b',
       lwd = linewidth,
       xlab='',ylab='')
  title(xlab = "iterations", line = axislabelspace)           # Add x-axis text
...
make_plot2<-function(){
  ...
  last_result = fitting_results[10,]
  phiPS2_new  = PhiPSII_func(leafT,c(last_result$sf1,1,1))
...
last_result = fitting_results[10,]
scaling_factors = c(last_result$sf1,last_result$sf2)
